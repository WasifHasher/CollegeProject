...
<div class="container mt-3" style="max-height:800px;">
    @if(session('status'))
<div class="alert alert-success mt-4">{{session('status')}}</div>
@endif
    {{-- <h3>Recieved Orders</h3> --}}
    <h3 class="text-black mt-5">{{$today}}</h3>
    ...
    <div class="row mt-3">
        ...
        @if($received_orders->isEmpty())
            <h2 class="text-black">There is no Order Record.</h2>
            @else
            @endif
        <div class="clearfix text-white mt-4 mx-2 py-2" style="background:#22222a;">
